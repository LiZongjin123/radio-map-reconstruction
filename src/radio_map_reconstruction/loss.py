...
def normalized_mse_per_sample(
    outputs: Tensor, targets: dict[str, Tensor]
) -> Tensor:
    gain = targets["gain"]
    mask = targets["mask"]
    ...
    return error_per_sample / valid_pixels_per_sample


class RadioMapLoss(Module):

    def forward(self, outputs: Tensor, targets: dict[str, Tensor]) -> Tensor:
        return normalized_mse_per_sample(outputs, targets).mean()
