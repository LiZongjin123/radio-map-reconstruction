...
class DeterministicSamplerEvaluationDataset(Dataset):
    def __len__(self) -> int:
        return len(RadioDataset.EVALUATION_SAMPLE_COUNTS)

    def __getitem__(self, index: int) -> tuple[Tensor, dict[str, Tensor]]:
        sample_count = RadioDataset.EVALUATION_SAMPLE_COUNTS[index]
...
class ControlledCheckpointModel(Module):
...
def test_eval_sampler_loads_only_independent_best_checkpoints(
    ...
    monkeypatch.setattr(eval_module, "ResUnet", ControlledCheckpointModel)
    monkeypatch.setattr(sampler_eval_module, "Sampler", ControlledCheckpointModel)
    monkeypatch.setattr(
        sampler_eval_module, "RadioDataset", ControlledRadioDataset
    )
    captured = []
    ...
    monkeypatch.setattr(
        sampler_eval_module, "run_sampler_evaluation", capture_evaluation
    )
    ...
    assert evaluation["test_dataset"].part == "test"
    assert evaluation["evaluation_dir"] == sampler_dir / "evaluation"
    assert evaluation["reconstructor"].weight.item() == 2.0
    assert evaluation["sampler"].weight.item() == 3.0
    assert evaluation["sampler"].model_config == {
        "in_channels": 2,
        "out_channels": 1,
        "base_channels": 3,
    }
