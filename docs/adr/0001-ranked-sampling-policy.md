...
Use the hard binary learned sampling mask in the forward pass. During backpropagation, use the gradient of a sigmoid soft Top-K surrogate. Solve an adaptive threshold per sample by bisection, treat that threshold as constant during differentiation, and configure the positive temperature, tolerance, and maximum iteration count.
...
- The estimator is biased, and gradients do not include the dependence of the adaptive threshold on the scores.
