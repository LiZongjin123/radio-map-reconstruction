...
从重建器的测试案例可以观察到，重建信道地图在增益变化剧烈处误差较大。所以，在大梯度位置增加采样点数量可以提高重建信道地图精度。
...
通过以下公式，将粗糙重建信道地图转化为增益梯度图：

$$
\nabla \mathbf{H}_c(x, y)=\sqrt{\frac{\partial H_c}{\partial x}^2+\frac{\partial H_c}{\partial y}^2}
$$

其中，$\mathbf{H}_c$ 表示粗糙重建信道地图
